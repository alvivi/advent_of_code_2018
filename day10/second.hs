...
find :: Bag (Int, Int) (Int, Int) -> (Int, Bag (Int, Int) (Int, Int))
find bag =
  let
    findStep 0 _ _ bestState _ = bestState
    findStep fails bestScore stepCount best@(bestStepCount, bestState) state =
      let
        score = heuristic state
      in
        if score < bestScore then
          findStep (fails - 1) bestScore (stepCount + 1) best $ step state
        else
          findStep fails score (stepCount + 1) (stepCount + 1, state) $ step state
  in
    findStep 100 (heuristic bag) 0 (0, bag) (step bag)
...
main = do
  ...
  let (count, bestState) = find state
  putStrLn $ bagShow $ bestState
  putStrLn $ "t(s) " ++ show count
